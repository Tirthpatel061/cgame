    ...
    enterDoor: {
      ...
      onComplete: () => {
        ...
        gsap.to(overlay, {
          ...
          onComplete: () => {
            if (level === 1) {
              // If the player enters the door, redirect to indexgame3.html (arrays game)
              window.location.href = "indexgame3.html"; // Navigate to arrays game
            } else {
              level++;
              if (level === 4) level = 1;
              levels[level].init();
