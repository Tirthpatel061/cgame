    ...
    enterDoor: {
      ...
      onComplete: () => {
        ...
        gsap.to(overlay, {
          ...
          onComplete: () => {
            if (level === 1) {
              // If the player enters the door, redirect to indexgame4.html (functions game)
              window.location.href = "indexgame4.html"; // Navigate to functions game
            } else {
              level++;
              if (level === 4) level = 1;
              levels[level].init();
