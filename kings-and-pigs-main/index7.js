    ...
    enterDoor: {
      ...
      onComplete: () => {
        ...
        gsap.to(overlay, {
          ...
          onComplete: () => {
            if (level === 1) {
              // If the player enters the door, redirect to indexgame7.html (structures game)
              window.location.href = "indexgame7.html"; // Navigate to structures game
            } else {
              level++;
              if (level === 4) level = 1;
              levels[level].init();
...
// Make canvas transparent (if not already set in CSS)
  ...
  3: {
    init: () => {
      ...
      doors = [
        ...
        new Sprite({ position: { x: 350, y: 362 }, imageSrc: './img/doorOpen.png', frameRate: 5, frameBuffer: 5, loop: false, autoplay: false }),
