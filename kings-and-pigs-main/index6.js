    ...
    enterDoor: {
      ...
      onComplete: () => {
        ...
        gsap.to(overlay, {
          ...
          onComplete: () => {
            if (level === 1) {
              // If the player enters the door, redirect to indexgame6.html (strings game)
              window.location.href = "indexgame6.html"; // Navigate to strings game
            } else {
              level++;
              if (level === 4) level = 1;
              levels[level].init();
