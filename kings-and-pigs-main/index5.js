    ...
    enterDoor: {
      ...
      onComplete: () => {
        ...
        gsap.to(overlay, {
          ...
          onComplete: () => {
            if (level === 1) {
              // If the player enters the door, redirect to indexgame5.html (pointers game)
              window.location.href = "indexgame5.html"; // Navigate to pointers game
            } else {
              level++;
              if (level === 4) level = 1;
              levels[level].init();
